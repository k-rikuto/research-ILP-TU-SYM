...
SLCなし、完全単模性を保持せずに、対称性を完全に削除したRWAモデル
    ...
    subject to
    <流量保存制約>
    ∑_(ω∈W)▒∑_(e∈L_(s_r)^-)▒α_r^(e,ω) -∑_(ω∈W)▒∑_(e∈L_(s_r)^+)▒α_r^(e,ω) =-1,∀r∈R
    ∑_(ω∈W)▒∑_(e∈L_(d_r)^-)▒α_r^(e,ω) -∑_(ω∈W)▒∑_(e∈L_(d_r)^+)▒α_r^(e,ω) =1,∀r∈R
    ∑_(e∈L_v^-)▒α_r^(e,ω) -∑_(e∈L_v^+)▒α_r^(e,ω) =0,∀r∈R,v∈V∖{s_r,d_r },ω∈W
    <波長非重畳制約>
    ∑_(r∈R)▒α_r^(e,ω) ≤β^ω,∀e∈E,ω∈W
    <W_maxの下限>
    ωβ^ω≤W_max,∀ω∈W
...
def ILP_RWA_01(graph:nx.Graph, R:dict[int,tuple[int,int]], W:set[int], timelimit=0, restart=False) -> tuple[bool,float,int,dict[int, tuple[tuple[int, int], set[tuple[int, int, int]], int]],int]:
    ...
    if restart:
        ...
        for r in R:
            for e in E_DIR:
                for w in W:
                    alpha[r,e,w] = model.getVarByName(name=f"alpha_{r}_{u}_{v}_{w}")
        beta = {}
        for w in W:
            beta[w] = model.getVarByName(name=f"beta_{w}")

        w_max = model.getVarByName(name="w_max")
        ...
        for r in R:
            for e in E_DIR:
                (u,v) = e
                for w in W:
                    alpha[r,e,w] = model.addVar(vtype=gp.GRB.BINARY, name=f"alpha_{r}_{u}_{v}_{w}")
        beta = {}
        for w in W:
            beta[w] = model.addVar(vtype=gp.GRB.BINARY, name=f"beta_{w}")

        w_max = model.addVar(vtype=gp.GRB.INTEGER, name="w_max")
        ...
        for r in R:
            src,dest = R[r]
            model.addConstr(gp.quicksum(alpha[r,e,w] for e in L_incoming[src] for w in W)-gp.quicksum(alpha[r,e,w] for e in L_outgoing[src] for w in W)==-1)
            model.addConstr(gp.quicksum(alpha[r,e,w] for e in L_incoming[dest] for w in W)-gp.quicksum(alpha[r,e,w] for e in L_outgoing[dest] for w in W)==1)
            for v in V-{src,dest}:
                for w in W:
                    model.addConstr(gp.quicksum(alpha[r,e,w] for e in L_incoming[v])-gp.quicksum(alpha[r,e,w] for e in L_outgoing[v])==0)
    ...
    # runtime, w_usedを返す。getPathがTrueの時、pathとw_allocも返す
        ...
        for w in W:
            if beta[w].X > EPS:
                w_used += 1
        data, w_max = convert_vars_to_data(alpha=alpha, E_DIR=E_DIR, R=R, W=W)
    ## 何かしらのエラーが発生した際の処理
    else:
        print("何かしらのエラーが発生")
