...
def ILP_RCWA_03(graph:nx.Graph, R:dict[int,tuple[int,int]], W:set[int], C:set[int], timelimit=0, restart=False) -> tuple[bool,float,int,dict[int, tuple[tuple[int, int], set[tuple[int, int, int]], int]],int]:
    ...
    if timelimit == 0:
        ...
        env.setParam('LogFile', "results/Logs/RCWA_03.log")
        env.start()
    else:
        env = gp.Env()
        env.setParam('OutputFlag', 1)
    ...
    if restart:
        ...
        model = gp.Model("ILP_RCWA_03",env=env)
