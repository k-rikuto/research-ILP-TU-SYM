...
def ILP_RCWA_01(graph:nx.Graph, R:dict[int,tuple[int,int]], W:set[int], C:set[int]):
    ...
    for r in R:
        for c in C:
            gamma[r,c] = model.addVar(vtype=GRB.BINARY, name=f"request {r} use core {c}")
    ...
    for r in R:
        for e in E_DIR:
            for c in C:
                model.addConstr(quicksum(alpha[r,e,w,c] for w in W)<=gamma[r,c])
    
    # コアの連続制約
    for r in R:
        model.addConstr(quicksum(gamma[r,c] for c in C)==1)
    ...
    return path, w_alloc, c_alloc, w_max.X
...
path,w_alloc,c_alloc,w_max = ILP_RCWA_01(graph=graph, R=R, W=W, C=C)
# for r in R:
#     print("----------------------------")
#     print(f"リクエスト{r}")
#     print(f"パス：{path[r]}")
#     print(f"コア：{c_alloc[r]}")
